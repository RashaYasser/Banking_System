#include "ReceivedResponseHandler.h"
...
ReceivedResponseHandler::ReceivedResponseHandler(const QString &action)
{
    ...
    {
        response = handleGetAccountNumberUser(request);
    }
    else
    {
        handler = nullptr;
...
QJsonObject LoginHandler::handle(const QJsonObject &request, DataBaseManager &dbManager)
{
    ...
    QJsonObject user = databaseManager.getUser(username);
...
QJsonObject UpdateUserHandler::handle(const QJsonObject &request, DataBaseManager &dbManager)
{
    ...
    QJsonArray accounts = databaseManager.getAllAccounts();
    ...
    bool success = databaseManager.updateUser(accountNumber, updatedUser);
...
QJsonObject DeleteUserHandler::handle(const QJsonObject &request, DataBaseManager &dbManager)
{
    ...
    bool success = databaseManager.deleteUser(accountNum);
    response["Status"] = success ? "Success" : "Failure";
    response["Action"] = "deleteUser";
...
QJsonObject TransferAmountHandler::handle(const QJsonObject &request, DataBaseManager &dbManager)
{
    ...
    QJsonObject senderAccount = databaseManager.getUser(sender);
    QJsonObject recipientAccount = databaseManager.getUser(recipient);
    ...
    bool transferResult = databaseManager.transferAmount(sender, recipient, amount);
    if (!transferResult)
    {
        response["Status"] = "Failure";
...
QJsonObject MakeTransactionHandler::handleResponse(const QJsonObject &request, DataBaseManager &dbManager)
{
    QJsonObject response;
    ...
    bool success = databaseManager.addTransaction(accountNumber, transaction);
...
QJsonObject GetAccountNumberAdminHandler::handle(const QJsonObject &request, DataBaseManager &dbManager)
{
    ...
    QJsonObject userAccount = databaseManager.getUser(username);
...
QJsonObject GetAccountNumberUserHandler::handle(const QJsonObject &request, DataBaseManager &dbManager)
{
    ...
    if (!request.contains("UserName"))
    {
        ...
        qDebug() << "Response:" << QJsonDocument(response).toJson();
